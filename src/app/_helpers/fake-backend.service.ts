  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    function handleRoute() {
      switch (true) {
        ...
        case url.match(/\/users\/\d+$/) && method === 'PUT':
          return updateUser();
        case url.match(/\/users\/updatepass\/\d+$/) && method === 'PUT':
    ...
    function error(message: string) {
      return throwError(() => ({ error: { message } }))
        .pipe(materialize(), delay(500), dematerialize()); // call materialize and dematerialize to
      // ensure delay even if an error is thrown (https://github.com/Reactive-Extensions/RxJS/issues/648);
    }
